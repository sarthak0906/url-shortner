...
2. Rename `config.example.js` to `config.js` in both server and client folders and fill them properly.
3. Install dependencies: `npm install`.
4. Start Neo4j database and properly fill the login data in config files.
...
URL {
  ...
  shortUrl {string} The shortened link (Usually https://localhost:3000/id for server running on localhost), This can be changed by changing the default domain in config folder.
}
```
